...
namespace WMS
{
    public partial class DeclarePhysicalCountWindow : Form
    {
        ...
        {
            DataTable dt = DataSupport.RunDataSet(@"SELECT * FROM PhysicalCountDetailItems WHERE phcount = '" + phcount_id + "' AND product !='EMPTY'").Tables[0];
            foreach (DataRow row in dt.Rows)
                header_grid.Rows.Add(row["location"], row["product"], row["uom"], row["lot_no"], DateTime.Parse( row["expiry"].ToString()).ToShortDateString(),"", "YES");
            ...
            if (dialog.ShowDialog() == DialogResult.OK)
            {
                ...
                if (select_dialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    if (select_location_dialog.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        if (!is_in_grid)
                        {
                            var row_index = header_grid.Rows.Add(location, dialog.product, dialog.uom, selected_row.Cells["Lot No"].Value.ToString(), selected_row.Cells["Expiry"].Value.ToString(), 1);
