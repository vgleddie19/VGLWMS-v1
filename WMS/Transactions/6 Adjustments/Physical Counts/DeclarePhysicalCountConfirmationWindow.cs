...
namespace WMS
{
    public partial class DeclarePhysicalCountConfirmationWindow : Form
    {
        ...
        private void DeclarePhysicalCountConfirmationWindow_Load(object sender, EventArgs e)
        {
            ...
            {
                dt.Rows.Add(parent.phcount_id
                    , row.Cells["product"].Value
                    , row.Cells["desc"].Value
                    , row.Cells["actual_qty"].Value
                    ,0
                    , row.Cells["uom"].Value
                    , row.Cells["lot_no"].Value
                    , row.Cells["expiry"].Value
                    , row.Cells["location"].Value
                    , row.Cells["expected"].Value);
            }
            ReportDocument rviewer = new ReportDocument();
            rviewer = new crtphysicalcount_declare();
            rviewer.SetDataSource(dt);
            if(parent.phcount_header_row["counted_by"].ToString().Trim().Length >= 1)
                rviewer.SetParameterValue("performby", parent.phcount_header_row["counted_by"].ToString());
            else
